...
def predict():
    ...
    if data == "":
        #params = request.form
        x_in = hyperjson.loads(request.form['image'])
        #x_in = json.loads(request.form['image'])
        width = int(request.form['w'])
        height = int(request.form['h'])
    else:
        print("????");
        params = hyperjson.loads(request.data)
        #params = json.loads(request.data)
        x_in = params['image']
        width = int(params['w'])
        height = int(params['h'])
        print("Time spent handling the parameter: %f" % (time.time() - start))
    ############
    #Convert PIL Image
    ######

    na = np.array(x_in, dtype=np.uint8)
    na = np.reshape(na, (3,height, width)).transpose(1,2,0)
    img = Image.fromarray(na, 'RGB')
    #img.save('./data/abc.jpg')
    ...
    json_data = json.dumps({'seg_label': seg_map.tolist(), 'w':len(seg_map[0]), 'h':len(seg_map)})
